        ...
        if result:
            ...
            for j in range(len(result_data)):
                ...
                if x > 0:
                    ...
                    self.connection.commit()
                    query = """
                        insert into payment_history (date,time,data_money,data_type,owner_id) values 
                        (%s,%s,%s,%s);
                        """
                    self.cur.execute(query, (date_today_strf(), time_today(), xien_5p * x, 'win_xs_5p', id_owner))
                    self.connection.commit()
